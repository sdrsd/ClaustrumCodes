...
% - Spatial tuning: the larger, the more spatially tuned
%alpha = [708,708,708,708];         % - point stimulus; 708 is max
%alpha = [20,20,20,20];             
%alpha = [5,5,5,5];                 
%alpha = [2,2,2,2];                 % - standard spatial tuning curve 
%alpha = [1,1,1,1];                 
%alpha = [0.5,0.5,0.5,0.5];         
alpha = [0,0,0,0];                  % - flat input at the level of the amplitude
...
%% - weight matrix: CC effects

% - local connectivity strength: CC
A = [.1,.33,.33, .33;               %PN->  PN PV SST VIP
    -.32,-.31,-.14, -0.02;          %PV->  PN PV SST VIP
    -.40, -.25, -.24, -0.02;        %SST-> PN PV SST VIP
    -0.01, -.07, -.03, -.004];      %VIP-> PN PV SST VIP

% - spatial connectivity parameters: CC
S = [0.15, 0.15, 0.15, 0.15; 
     0.17, 0.15, 0.14, 0.22; 
     0.24, 0.17, 0.33, 0.2; 
     0.18, 0.14, 0.21, 0.27]; 

% - input center offset (according to medio-lateral connectivity maps): CC
 d0_matrix = [0, 0, 0, 0                     
             0.007, 0.032, -0.005, 0.026    
             0.012, 0.024, 0.079, 0.038     
             0.019, -0.033, 0.016, 0.068];         
...
%% This section tests the inhibitory or disinhibitory effects of certain IN connections
% Toggle inhibition or disinhibition (but not both)

% - INHIBITION MOTIF
% By toggeling connections (1=on, 0=off), inhibitory connections could be
% removed or added to the network. W1 serves as a reference network with connections
% being implemented, W2 as test-network without connections switched on.

% Example for testing the inhibitory effects of PVs

% % - IN motif _________________  CHECK INTERNEURON_________________________
%fig_suffix = ['local CC network PV 2 inhibition_ alpha_', num2str(alpha(1))]   %'local CC network... PV inhibition'
fig_suffix = ['Fig2a_Model_local_CC'];

% %inhibition
W1 = [Wi{1,1}*1, Wi{1,2}*1, Wi{1,3}*0, Wi{1,4}*0;     
     Wi{2,1}*1, Wi{2,2}*0, Wi{2,3}*0, Wi{2,4}*0;      
     Wi{3,1}*0, Wi{3,2}*0, Wi{3,3}*0, Wi{3,4}*0;      
     Wi{4,1}*0, Wi{4,2}*0, Wi{4,3}*0, Wi{4,4}*0;      
     ]; 

W2 = [Wi{1,1}*1, Wi{1,2}*1, Wi{1,3}*0, Wi{1,4}*0; 
...
% - DISINHIBITION MOTIF (needs to be activated)
% By toggeling connections (1=on, 0=off), disinhibitory connections could be
% removed or added to the network. W1 serves as a reference network with connections
% being implemented, W2 as test-network without connections switched on.

% Example for testing the disinhibitory effects of PVs:
% (all IN and PV->IN) vs (all IN but no PV-> IN)
  
% % - IN motif _________________  CHECK INTERNEURON_________________________
% fig_suffix = ['local CC network PV 2 DISinhibition_ alpha_', num2str(alpha(1))]
% %disinhibition
% W1 = [Wi{1,1}*1, Wi{1,2}*1, Wi{1,3}*1, Wi{1,4}*1; 
%      Wi{2,1}*1, Wi{2,2}*1, Wi{2,3}*1, Wi{2,4}*1;  
%      Wi{3,1}*1, Wi{3,2}*0, Wi{3,3}*0, Wi{3,4}*0;   
%      Wi{4,1}*1, Wi{4,2}*0, Wi{4,3}*0, Wi{4,4}*0; 
%      ]; 
% 
% W2 = [Wi{1,1}*1, Wi{1,2}*1, Wi{1,3}*1, Wi{1,4}*1; 
%      Wi{2,1}*1, Wi{2,2}*0, Wi{2,3}*0, Wi{2,4}*0;  
%      Wi{3,1}*1, Wi{3,2}*0, Wi{3,3}*0, Wi{3,4}*0;   
%      Wi{4,1}*1, Wi{4,2}*0, Wi{4,3}*0, Wi{4,4}*0; 
%      ]; 
